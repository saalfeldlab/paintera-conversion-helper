...
class PainteraConvert {
    ...
    companion object {
        ...
        private val DEFAULT_BUILDER = GsonBuilder().setPrettyPrinting().disableHtmlEscaping()

        private const val LABEL_BLOCK_LOOKUP_KEY = "labelBlockLookup"

        private const val RAW_IDENTIFIER = "raw"

        private const val LABEL_IDENTIFIER = "label"

        private const val CHANNEL_IDENTIFIER = "channel"

        private const val CHANNEL_AXIS_KEY = "channelAxis"

        private const val CHANNEL_BLOCKSIZE_KEY = "channelBlockSize"

        private const val RESOLUTION_KEY = "resolution"

        private const val OFFSET_KEY = "offset"

        private const val PAINTERA_DATA_KEY = "painteraData"
        ...
        @Throws(IOException::class)
        private fun setPainteraDataType(writer: N5FSWriter, group: String, type: String) = writer.setAttribute(group, PAINTERA_DATA_KEY, mapOf(Pair(TYPE_KEY, type)))

        private fun N5Reader.getDoubleArrayAttribute(dataset: String, attribute: String) = try {
        ...
        private fun String.n5Reader() = N5Helpers.n5Reader(this)

        private fun String.n5Writer(builder: GsonBuilder? = null) = builder?.let { N5FSWriter(this, it) } ?: N5FSWriter(this)

        @JvmStatic
        fun main(args: Array<String>) {
            val info = DatasetInfo(
                    "/home/hanslovskyp/Downloads/sample_B_20160501.hdf",
                    "volumes/raw",
                    "abcdefghijklmnop.n5"
            )
            val conf = SparkConf()
            ...
            JavaSparkContext(conf).use { sc ->
                handleRawDatasetInferType(
                        ...
                        info,
                        intArrayOf(64, 64, 64),
                        arrayOf(intArrayOf(2, 2, 1)),
                        arrayOf(intArrayOf(64, 64, 64)),
                        true,
